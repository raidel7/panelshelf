...
test("skips are added, removed, and survive a restart", async (t) => {
  ...
  assert.deepEqual(store.list(READER), { nodeIds: [] });
  await store.apply(READER, { add: [NODE_A, NODE_B] });
  assert.deepEqual(store.list(READER).nodeIds.sort(), [NODE_B, NODE_A].sort());
  ...
  await store.apply(READER, { add: [NODE_A] });
  assert.equal(store.list(READER).nodeIds.length, 2);

  await store.apply(READER, { remove: [NODE_A] });
  assert.deepEqual(store.list(READER).nodeIds, [NODE_B]);

  const restarted = new SkipStore(directory);
  await restarted.initialize();
  assert.deepEqual(restarted.list(READER).nodeIds, [NODE_B]);
});

test("a request that both adds and removes a branch leaves it skipped", async (t) => {
  const { store } = await makeStore(t);
  await store.apply(READER, { add: [NODE_A], remove: [NODE_A] });
  assert.deepEqual(store.list(READER).nodeIds, [NODE_A]);
});
...
test("malformed skip requests are refused rather than half-applied", async (t) => {
  ...
  for (const bad of [null, "nope", [], 7]) {
    await assert.rejects(() => store.apply(READER, bad), /object/i);
  }
  await assert.rejects(() => store.apply(READER, { add: "not-a-list" }), /list of node ids/i);
  await assert.rejects(() => store.apply(READER, { remove: {} }), /list of node ids/i);

  assert.deepEqual(store.list(READER).nodeIds, [NODE_A], "nothing changed");
});
...
test("an oversized set is refused and leaves the store untouched", async (t) => {
  ...
  await assert.rejects(() => store.apply(READER, { add: flood }), /Too many/i);
...
test("a corrupt skip file is preserved and the store starts empty", async (t) => {
  ...
  const restarted = new SkipStore(directory);
  await restarted.initialize();
  assert.deepEqual(restarted.list(READER).nodeIds, []);
...
test("both the wrapped and bare forms are read back", async (t) => {
  ...
  await restarted.initialize();
  assert.deepEqual(restarted.list(READER).nodeIds, [NODE_A]);
