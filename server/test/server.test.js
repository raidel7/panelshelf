...
test("the support bundle downloads, and needs a token when pairing is on", async (t) => {
  ...
  assert.equal(bundle.panelshelf.version, "0.5.1");
...
test("a page nobody named cannot be framed, and the DSM window can", async (t) => {
  const plain = await startServer(t);
  let response = await fetch(`${plain.base}/api/health`);
  assert.equal(response.status, 200, plain.state.logs);
  // The default every release has shipped. PanelShelf has no accounts, so a
  // page that can frame it can take a click from somebody who already has it
  // open, and nothing should be able to until an owner says which.
  assert.equal(response.headers.get("x-frame-options"), "SAMEORIGIN", plain.state.logs);
  assert.ok(
    !response.headers.get("content-security-policy").includes("frame-ancestors"),
    plain.state.logs
  );

  const framed = await startServer(t, {
    env: { PANELSHELF_FRAME_ANCESTORS: "http://*:5000 https://*:5001" }
  });
  response = await fetch(`${framed.base}/api/health`);
  assert.equal(response.status, 200, framed.state.logs);
  assert.match(
    response.headers.get("content-security-policy"),
    /frame-ancestors 'self' http:\/\/\*:5000 https:\/\/\*:5001;/,
    framed.state.logs
  );
  assert.equal(
    response.headers.get("x-frame-options"),
    null,
    "and not contradicted by the older header beside it"
  );

  // The setting reaches the pages a browser actually loads, not just the API.
  const page = await fetch(`${framed.base}/`);
  assert.match(page.headers.get("content-security-policy"), /frame-ancestors/, framed.state.logs);
});

test("a frame-ancestors setting nobody can parse is refused out loud", async (t) => {
  const { base, state } = await startServer(t, {
    env: { PANELSHELF_FRAME_ANCESTORS: "http://nas:5000; script-src 'unsafe-inline'" }
  });

  const response = await fetch(`${base}/api/health`);
  assert.equal(response.status, 200, state.logs);
  // Refusing it is the safe outcome; saying so is what keeps it from looking
  // like the setting was applied.
  assert.equal(response.headers.get("x-frame-options"), "SAMEORIGIN", state.logs);
  assert.ok(!response.headers.get("content-security-policy").includes("unsafe-inline"), state.logs);
  assert.match(state.logs, /PANELSHELF_FRAME_ANCESTORS ignored/);
});
